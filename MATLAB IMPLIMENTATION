// matlab code for face match ditection 
// challange was to only use DSP concepts 
// without ML 
...
subplot(1,2,1), imshow(ref_img_raw), title('Reference 
Image'); 
...
sigma1_sq = imfilter(double(ref_img).^2, window, 
'replicate') - mu1.^2; 
sigma2_sq = imfilter(double(test_img).^2, window, 
'replicate') - mu2.^2; 
sigma12 = imfilter(double(ref_img) .* double(test_img), 
window, 'replicate') - mu1 .* mu2; 
ssim_map = ((2 * mu1 .* mu2 + C1) .* (2 * sigma12 + C2)) 
./ ... 
((mu1.^2 + mu2.^2 + C1) .* (sigma1_sq + sigma2_sq 
+ C2)); 
...
subplot(2,3,3), imshow(log(1 + abs(ref_dft)), []), 
title('DFT Spectrum (Reference)'); 
subplot(2,3,4), imshow(log(1 + abs(test_dft)), []), 
title('DFT Spectrum (Test)'); 
